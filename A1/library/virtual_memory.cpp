...
super_block read_super_block_from_vm(int fd){
    ...
    super_block* super_block_in_vm = mmap(NULL, 4096, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
check_point read_checkpoint_from_vm(int fd){
    ...
    check_point* checkpoint_in_vm = mmap(NULL, sizeof(check_point), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 2048000000);
